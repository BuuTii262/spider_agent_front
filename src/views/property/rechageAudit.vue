  ...
  <div class="wrap">
    <el-card style="margin-bottom: 10px">
      ...
      <el-form style=" margin-left: 10px;display: flex; align-items: center;margin:50px; flex-wrap: wrap;">
        ...
        <el-form-item label="币种" style="width:400px">
          <el-input v-model="coin_code" placeholder="请输入币种" style="width: 200px"></el-input>
        </el-form-item>
        <!-- 充值状态 -->
        ...
        <el-form-item label="时间" style="min-width:500px">
          <el-date-picker v-model="formOptions.TimeStart" type="datetime" placeholder="选择开始时间"
            style="width: 200px; margin-right: 30px" @change="optionsTimeStratChange"></el-date-picker>
          <el-date-picker v-model="formOptions.TimeEnd" type="datetime" placeholder="选择结束时间" style="width: 200px"
            @change="optionsTimeEndChange"></el-date-picker>
        </el-form-item>
    ...
    <el-card>
      <el-table :data="tableData" style="width: 100%" v-loading="loading" empty-text="暂无数据">
        <el-table-column prop="list1" label="入账时间" min-width="150" fit></el-table-column>
        ...
        <el-table-column prop="list19" label="用户名" min-width="200" fit></el-table-column>
        <el-table-column prop="list4" label="币种" min-width="100" fit></el-table-column>
        <el-table-column prop="amount" label="金额" min-width="100" fit></el-table-column>
        ...
        <el-table-column label="操作" min-width="120" fit align="center">
          <template slot-scope="scope">
            <el-button type="primary" size="small" @click="auditBtn(scope.row)" v-if="scope.row.list11!=='审核成功'">
    ...
    <!-- 充值审核 -->
    <el-dialog title="充值审核" :visible.sync="visible" width="40%" top="5vh" :before-close="hideCreate"
      :close-on-click-modal="false">
      <el-form ref="createForm" :model="createForm" label-width="120px" class="modifyForm">
        <el-form-item label="充值证明" prop="fileName">
          <el-image style="width: 150px; height: 150px" :src="createForm.fileName"
            :preview-src-list="Array(createForm.fileName)">
          </el-image>
        </el-form-item>
      ...
      <span slot="footer" class="dialog-footer">
        <el-button @click="submit('reject')" type="danger">拒绝</el-button>
...
<script>
import { auditList, auditHandel } from "@/api/audit";
...
export default {
  ...
  methods: {
    ...
    auditBtn(row) {
      this.visible = true;
      this.createForm.fileName = row.fileName;
    ...
    getList(offset) {
      ...
      auditList(data).then(({ err_code, msg, data }) => {
        if (err_code == "0" && data.total !== 0) {
    ...
    optionsTimeStratChange(val) {
      ...
      if (
        ...
        let start = Number(moment(this.formOptions.TimeStart).valueOf());
        let end = Number(moment(this.formOptions.TimeEnd).valueOf());
        if (start > end) {
          this.$message({
            type: "warning",
